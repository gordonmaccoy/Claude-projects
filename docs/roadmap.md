...
- [ ] Supabase project created; initial migration applied.
- [ ] Next.js scaffold pushed to GitHub and Vercel; empty site deploys at a placeholder domain.
- [ ] Kakao developer account + REST API key + JS SDK key obtained; domain whitelisted.
...
- Schema migration + RLS policies applied to Supabase.
- `scripts/seed-from-kakao.ts` produces ≥50 Seoul restaurant drafts.
- Curation pass: every live row has style, dish tags, price, rating, and bilingual curator note.
- Home page: Kakao Map + filter bar (style, dish tags, price) + listing list. Responsive.
- Restaurant detail page with SSR and SEO metadata.
- `/suggest` form writes to `submissions` with IP rate-limiting.
- i18n wired with `/ko` and `/en`; language toggle in header.
...
6. **Taco Feed** — user posts (photo + short text + tagged restaurant), chronological. Adds `feed_posts` table.
7. **Taco Hub** — community landing: top-rated restaurants, most-active users, featured crawls.
8. **Taco Blog** — editorial long-form. Start as MDX in the repo (no CMS). Introduce `blog_posts` only when editorial team grows.
